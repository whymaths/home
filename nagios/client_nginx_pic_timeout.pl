...
my $error_log_file = "/opt/app/nginx/logs/error.log";
my $saved_file = "/tmp/client_nginx_pic";
...
my $client_nginx_pic_met = 0;

my $array_length = $#saved_status_array;
for my $line (0..$array_length) {
    if ($saved_status_array[$line] =~ m/(\d+)\:\ client_nginx_pic\:\ (\d+)/xms) {
        $error_log_file_inode_last_check = $1;
        $error_log_file_offset_last_check = $2;
        $saved_status_array[$line] = "$error_log_file_inode: client_nginx_pic: $error_log_file_size\n";
        $client_nginx_pic_met = 1;
    }
}

push @saved_status_array, "$error_log_file_inode: client_nginx_pic: $error_log_file_size\n" unless $client_nginx_pic_met;
...
map {
    ...
    if ($line =~ m/110\:\ Connection\ timed\ out/gxms) {
        $timeout_count++;
    }
